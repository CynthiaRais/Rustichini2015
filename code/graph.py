...
class Graph:
    def __init__(self, ΔA=20, ΔB = 20, t_exp =2.0, dt= 0.0005, analysis=None):
        ...
        self.X_axis = np.arange(-0.5, 1.0, self.dt)
    ...
    def firing_specific_set_ov(self, ov_choice, pourcentage_choice_B):
        figure_4_C = bokeh.plotting.figure(title="Figure 4 C", plot_width=300, plot_height=300)
    ...
    def firing_offer_B(self, firing_D):
        ...
        figure_4_D = bokeh.plotting.figure(title="Figure 4 D", plot_width=300, plot_height=300)
    ...
    def firing_specific_set_cjb(self, cj_choice, pourcentage_choice_B):
        figure_4_G = bokeh.plotting.figure(title="Figure 4 G", plot_width=300, plot_height=300)
    ...
    def firing_choice(self, firing_H):
        Y = firing_H
        figure_4_H = bokeh.plotting.figure(title="Figure 4 H", plot_width=300, plot_height=300)
    ...
    def firing_specific_set_cv(self, cv_choice, pourcentage_choice_B):
        figure_4_K = bokeh.plotting.figure(title="Figure 4 K", plot_width=300, plot_height=300)
    ...
    def firing_chosen_value(self, Y):
        figure_4_L = bokeh.plotting.figure(title="Figure 4 L", plot_width=300, plot_height=300)
    ...
    def test(self, firing_cj, firing_cv):
        figure_test_cj = bokeh.plotting.figure(title="Figure test cj a", plot_width=300, plot_height=300)
        figure_test_cj.line(x=np.arange(-1, 1, 0.0005 ), y=firing_cj, color="red")
        figure_test_cv = bokeh.plotting.figure(title="Figure test cj a", plot_width=300, plot_height=300)
        figure_test_cv.line(x=np.arange(-1, 1, 0.0005), y=firing_cv, color="red")
        bokeh.plotting.show(figure_test_cj)
