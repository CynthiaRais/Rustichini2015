...
class DataAnalysis:
    ...
    def mean_firing_rate_ovb(self):
        ...
        for i in range(0, self.ΔA + 1):
            for j in range(0, self.ΔB + 1):
                q = int(3 * j / (self.ΔB+1))
                for choice_i in ['A', 'B']:
                    if len(self.means_choice[(i, j, choice_i)]) > 0:
                        for k in range_k:
                            ovb_rates[q][k-1000].append(self.means_choice[(i, j, choice_i)]['r_ovb'][k])
    ...
    #             q = int(3 * j / (self.ΔB+1))
    #             for choice_i in ['A', 'B']:
    #                 if len(self.history.trials[(i, j, choice_i)][0]) == 0:
    #                     for k in range_k:
    #                         ovb_rates[q][k].append(self.result[(i, j, choice_i)][0][k])
    #
    #     for ovb_q in ovb_rates:
    #         for k, values in enumerate(ovb_q):
    #             ovb_q[k] = np.mean(values)
