...
class ReplicableTests(unittest.TestCase):
    ...
    def test_replicable(self, verbose=VERBOSE, x_a=1,x_b=0):
        ...
        np.testing.assert_almost_equal(access_data(datamat, 'nuOV1')[:K], model.trial_history.r_ova[:K])
        np.testing.assert_almost_equal(access_data(datamat, 'nuOV2')[:K], model.trial_history.r_ovb[:K])

        np.testing.assert_almost_equal(access_data(datamat, 'sampa1')[:K], model.trial_history.S_ampa_1[:K])
        np.testing.assert_almost_equal(access_data(datamat, 'Istim1')[:K], model.trial_history.I_stim_1[:K])

        np.testing.assert_almost_equal(access_data(datamat, 'phi1')[:K], model.trial_history.phi_1[:K])
